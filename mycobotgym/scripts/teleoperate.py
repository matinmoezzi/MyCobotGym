...
class Controller():
    ...
    def _rot(self, axis: int, direction: Direction):
        ...
        if axis == 2:
            ...
            self.sim.data.set_mocap_quat("mocap", np.array(
    ...
    def _move(self, axis: int, direction: Direction):
        ...
        if axis == 2:
            self.sim.data.set_mocap_pos(
                "mocap", self.sim.data.mocap_pos + np.array([0, 0, self.pos_speed * direction.value]))
            self.sim.step()
        elif axis == 1:
            self.sim.data.set_mocap_pos(
                "mocap", self.sim.data.mocap_pos + np.array([0, self.pos_speed * direction.value, 0]))
            self.sim.step()
        elif axis == 0:
            self.sim.data.set_mocap_pos(
                "mocap", self.sim.data.mocap_pos + np.array([self.pos_speed * direction.value, 0, 0]))
            self.sim.step()
        else:
            pass
...
def main():
    ...
    # viewer set up
    viewer = Viewer(sim)
    body_id = sim.model.body_name2id('gripper_base')
    ...
    # postion offset
    sim.data.set_mocap_pos("mocap", np.array([0.0138673, 0.21864658, 1.21236]))
